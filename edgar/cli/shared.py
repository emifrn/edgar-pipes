...
#
#
#def packet_ok(data_type: str, data: dict) -> str:
#    """Create successful packet record in envelope format."""
#    return json.dumps({"ok": True, "type": data_type, "data": data})
#
#
#def packet_err(message: str) -> str:
#    """Create error packet record in envelope format."""
#    return json.dumps({"ok": False, "type": "error", "data": message})
#
#
#def packet_read() -> Result[list[tuple[str, dict]], str]:
#    """
#    Read packet format from stdin.
#    
#    Returns:
#        Ok(list[dict]) - Successfully extracted data records
...
#        - Extracts data from packets
#    """
#    if sys.stdin.isatty():
#        return ok([])  # No piped input
#    
#    data = []
#    
...
#            return err(f"shared.packet_read: line {line_no}: invalid JSON - {e}")
#        
#        # Validate packet structure
#        if not isinstance(packet, dict):
#            return err(f"shared.packet_read: line {line_no}: expected JSON object, got {type(packet).__name__}")
#        
#        if "ok" not in packet:
#            return err(f"shared.packet_read: line {line_no}: missing 'ok' field in packet")
#        
#        # Check for error
#        if not packet["ok"]:
#            error_msg = packet.get("data", "unknown packet error")
#            return err(str(error_msg))
#        
#        # Extract data from success packet
#        if "data" not in packet:
#            return err(f"shared.packet_read: line {line_no}: missing 'data' field in packet")
#        
#        data.append((packet["type"], packet["data"]))
#    
#    return ok(data)
